...
python python/step1_makeHisto.py --inputDat=data/config.dat --nJobs=100 --jobId=$i 2>&1 | gzip > $PWD/log/log_$i.txt.gz && touch $PWD/log/log_$i.done || { touch $PWD/log/log_$i.fail; exit 1; }

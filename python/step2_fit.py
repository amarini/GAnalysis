...
def FIT(file,nJets=1,Ht=0,doShapeCorrFit=0,fileMC=ROOT.TFile.Open("/dev/null")):
		...
		if doShapeCorrFit:
			...
			if Bin == SigBin:
				try:
					...
					TruthMC.append(fileMC.Get("photoniso_MATCHED_VPt_%.0f_%.0f_Ht_%.0f_%.0f_phid_%.3f_%.3f_nJets_%d"%(PtCuts[p],PtCuts[p+1],Ht,8000,SigPhId[0],SigPhId[1],nJets) ) )
					TruthMC[-1].Divide(SigMC)
					TruthMC[-1].SetName("SigShapeCorrFit_VPt_%.0f_%.0f_Ht_%.0f_%.0f_phid_%.3f_%.3f_nJets_%d"%(PtCuts[p],PtCuts[p+1],Ht,8000,SigPhId[0],SigPhId[1],nJets)  )
					...
					SigCorr[-1].Multiply(TruthMC[-1])
				except AttributeError:
					doShapeCorrFit=0 # Turn Off Local var
			#BKG
			#BkgMC=fileMC.Get("photonisoRC_VPt_%.0f_%.0f_Ht_%.0f_%.0f_phid_%.3f_%.3f_nJets_%d"%(PtCuts[p],PtCuts[p+1],Ht,8000,BkgPhId[0],BkgPhId[1],nJets) )
	...
	for p in range(0,len(PtToFit)-1):
		...
		if doShapeCorrFit and NormSigCorr == 0 :
		...
		if doShapeCorrFit:
			...
			fCorr=ROOT.FIT.fit(ToFitTemplate[p],SigCorr[Sbin],BkgTemplate[Bbin],WorkDir+"/fitresults.root","Bin_PT_"+str(PtToFit[p])+"_"+str(PtToFit[p+1])+"_HT_"+str(Ht) +"_nJets_"+str(nJets) + ROOT.Analyzer.SystName(ROOT.Analyzer.SIGSHAPE) )
		...
		#Write output
		if doShapeCorrFit:
			o_txt.write("Pt "+str(PtToFit[p])+" "+str(PtToFit[p+1])+" Ht " +str(Ht) + " nJets "+ str(nJets)+ " Fraction= "+str(f) + " "+ROOT.Analyzer.SystName(ROOT.Analyzer.SIGSHAPE)+" "+str(fCorr))
		else:
		...
		if doShapeCorrFit:
			...
			TruthMC[Sbin].Write("",ROOT.TObject.kOverwrite)	
